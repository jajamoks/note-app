    ...
    <div>
        ...
        <div v-else>
            <div class="flex justify-between">
                ...
                <div class="relative">
                    <router-link :to="'/notes/' + note.note_id + '/edit'" class="px-4 py-2 rounded text-green-500 border border-green-500 text-sm font-bold mr-2">Edit</router-link>
            ...
            <div class="flex items-center pt-6">
                <UserCircle :name="note.name" />

                <p class="pl-5 text-xl">{{ note.name }}</p>
            </div>

            <p class="pt-6 text-gray-600 font-bold uppercase text-sm">Description</p>
            <p class="pt-2 text-blue-400">{{ note.description }}</p>
...
<script>
    ...
    export default {
        ...
        methods: {
            destroy: function () {
                axios.delete('/api/notes/' + this.$route.params.id)
                    .then(response => {
                        this.$router.push('/notes');
                    })
                    .catch(error => {
                        alert('Internal Error. Unable to delete note.');
                    });
            }
        }
    }
